...
class film_calibrationLogic(ScriptedLoadableModuleLogic):
    ...
    def create_calibration(self, volume_node, roi_nodes, calibration_file_path, output_dir_path):
        ...
        interpolation_parameters = self.__calculate_interpolatation_parameters(calibration_dict, roi_rgb_mean_normalized, roi_rgb_std_normalized)
        self.__create_interpolation_plot(calibration_dict, roi_nodes_data_normalized, interpolation_parameters, output_dir_path)
        
        with open(os.path.join(output_dir_path, 'calibration_parameters.json'), 'w') as f:
    ...
    def __create_interpolation_plot(self, calibration_dict, roi_nodes_data_normalized, interpolation_parameters, output_dir_path):
        def model_func(x, a, b, c):
            return (a + b * x) / (c + x)
